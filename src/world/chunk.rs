...
impl Chunk {
    pub fn init(
        ...
        let position_of_chunk = [
            (position[0] + end_pos[0]) / 2.0,
            (position[1] + end_pos[1]) / 2.0,
            (position[2] + end_pos[2]) / 2.0,
        ];
    ...
    pub fn regenerate(
        ...
        let position = [
            self.position[0] + half_chunk_width - 0.5,
            self.position[1],
            self.position[2] + half_chunk_width - 0.5,
        ];
            ...
            if self.visible_layers[j] {
                for i in 0..self.blocks.len() {
                    for k in 0..self.blocks[i].len() {
                        block::Block::get_mesh(
                            &self.blocks[i][k][j],
                            &mut self.vertices,
                            block_model,
                            &mut self.transparencies,
                        );
                    }
                }
            }
    ...
    pub fn populate_mesh(&mut self, display: &glium::Display<WindowSurface>) {
        ...
        self.vertex_non_transparent =
            Some(glium::VertexBuffer::new(display, &non_transparent_vertices).unwrap());
        self.vertex_transparent =
            Some(glium::VertexBuffer::new(display, &transparent_vertices).unwrap());
        self.vertices.clear();
    }
...
fn generate_chunk(
    ...
    chunk_i: usize,
    chunk_k: usize,
    blocks: &mut Vec<Vec<Vec<block::Block>>>,
    square_chunk_width: usize,
    position: [f32; 3],
    grid_x: i32,
    grid_z: i32,
    ...
    let mut columns: Vec<Vec<(u8, bool, f32, f32, u8)>> = vec![];

    let mut fbm = Fbm::<Perlin>::default().set_seed(world_gen_seed);
    ...
    for i in 0..square_chunk_width {
        ...
        for k in 0..square_chunk_width {
            if square_chunk_width <= 8
                && (i == 0 && k == 0
                    || i == 0 && k == square_chunk_width - 1
                    || i == square_chunk_width - 1 && k == 0
                    || i == square_chunk_width - 1 && k == square_chunk_width - 1)
                ...
                || square_chunk_width == 16 && (i % 3 == 0 && k % 3 == 0)
            {
                ...
                let value_worley =
                    (worley.get([(z as f64 + grid_z as f64), (x as f64 + grid_x as f64)]) + 1.0)
                        / 2.0;
                // 0 normal
                // 1 Ocean
                // 2 Dessert
                ...
                if value_worley > 0.97 {
                    ...
                    fbm.frequency = 0.02;
                    let value_fbm =
                        (fbm.get([(z as f64 + grid_z as f64), (x as f64 + grid_x as f64)]) + 1.0)
                            / 2.0;
                    ...
                    let value_fbm =
                        (fbm.get([(z as f64 + grid_z as f64), (x as f64 + grid_x as f64)]) + 1.0)
                            / 2.0;
                    max = map_value(value_fbm.powf(0.5) / 2.0, 0, mid_height) as u8
                        + underground_height;
                    type_biome = 2
                    ...
                    let value_fbm =
                        (fbm.get([(z as f64 + grid_z as f64), (x as f64 + grid_x as f64)]) + 1.0)
                            / 2.0;
                    max = map_value(f64::powi(value_fbm, 3), 0, mid_height) as u8
                        + underground_height;
                    type_biome = 1
                } else {
                    fbm.octaves = 6;
                ...
                let mut rng = rand_xoshiro::SplitMix64::seed_from_u64(
                    world_gen_seed as u64
                        + f64::powi(x as f64, 2) as u64
                        + f64::powi(z as f64, 4) as u64,
                );
                if type_biome != 2 && rng.random_range(1..50) == 1 {
                    has_tree = true;
                    if max > water_level + 2 {
                        trees.push((
                            ...
                            i,
                            k,
                            (max + underground_height + 6) as usize,
                        ));
                    }
                }
                columns[i].push((max, has_tree, z, x, type_biome));
            } else {
                columns[i].push((0, false, 0.0, 0.0, 0));
            }
        }
    }

    let coll_temp = columns.clone();
    ...
    for i in 0..square_chunk_width {
        ...
        for k in 0..square_chunk_width {
            if columns[i][k].0 == 0 {
                ...
                let max: u8 = (next_height * (before_distance / (before_distance + next_distance))
                    + before_height * (next_distance / (before_distance + next_distance)))
                    .round() as u8;
                ...
                columns[i][k] = (max, has_tree, z, x, type_biome);
            }
        }
    }
    let coll_temp = columns.clone();
    ...
    if !overwrite {
        for i in 0..square_chunk_width {
            let column: Vec<Vec<block::Block>> = vec![];
            blocks.push(column);
            for k in 0..square_chunk_width {
    ...
    (blocks).par_iter_mut().enumerate().for_each(|(i, val)| {
        ...
        for k in 0..val.len() {
            ...
            for j in 0..val[k].len() {
                ...
                if number == 241 {
                    number = get_block_type(
                        j as u8,
                        underground_height + column_values.0,
                        water_level,
                        column_values.1,
                        underground_height,
                        sky_height,
                        mid_height + underground_height + sky_height,
                        column_values.4,
                    );
                }

                val[k as usize][j as usize].regenerate(
                    [
                        column_values.3,
                        position[1] + 1.0 * j as f32,
                        column_values.2,
                    ],
                    number,
                );
    ...
    for i in 0..trees.len() {
        set_tree(
            ...
            trees[i].4,
            chunk_distance,
            (mid_height + underground_height + sky_height) as usize,
        );
    }

    return blocks[blocks.len() - 1][blocks[blocks.len() - 1].len() - 1][0]
        .position
        .clone();
}
...
fn get_block_type(
    ...
    if biome == 2 {
        ...
        //Check if j above column max
        if block_height > max_column_height {
        ...
        if max_column_height - block_height < 3 && block_height > water_level + 2 {
            return 6; //Sand
        }

        return 1; // Stone
    } else {
        //Check if the column has a plant, j is above water, the max height of the column is 2 blocks above water. J is up to 6 blocks bellow max column height and that j is aboce max column height
        if has_plant
            && block_height > water_level
            && water_level + 2 < max_column_height
            && block_height < max_column_height + 6
            && block_height > max_column_height
        {
            return 5; // Wood log
        }
...
fn set_tree_block(
    ...
    if j_max > j {
        if i < 0
            || k < 0
            || i > (blocks[0].len() - 1) as isize
            || k > (blocks[0].len() - 1) as isize
            || j > (blocks[0][0].len() - 1)
        {
            ...
            if (chunk_distance as i32 - 1) >= chunk_k_mut
                && (chunk_distance as i32 - 1) >= chunk_i_mut
                && 0 <= chunk_k_mut
                && 0 <= chunk_i_mut
            {
                change_block.push((
                    chunk_i_mut as usize,
                    chunk_k_mut as usize,
                    i_set as usize,
                    k_set as usize,
                    j,
                    7,
                ...
                let key = [
                    grid_x_set.to_string(),
                    grid_z_set.to_string(),
                    i_set.to_string(),
                    k_set.to_string(),
                    j.to_string(),
                ]
