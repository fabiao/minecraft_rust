...
pub struct World {
    ...
    pub loaded_textures: Option<glium::texture::SrgbTexture2d>,
...
impl World {
    pub fn new(
        ...
        square_chunk_width: &usize,
        chunks_layers_from_player: &usize,
        world_gen_seed: &u32,
        mid_height: &u8,
        underground_height: &u8,
        sky_height: &u8,
    ) -> World {
        let mut world = World {
            chunk_width: square_chunk_width.clone(),
            loaded_textures: Some(glium::texture::SrgbTexture2d::empty(display, 1, 1).unwrap()),
            chunk_grid: vec![],
            block_model: block_model::BlockModel::init(),
            view_distance: ((chunks_layers_from_player - 1) / 2 * square_chunk_width
                + square_chunk_width) as f32,
        ...
        generate_chunks(
            display,
            &mut world.chunk_grid,
            &mut world.change_block,
            &camera_position,
            &square_chunk_width,
            &chunks_layers_from_player,
            world_gen_seed,
            mid_height,
            &mut world.set_blocks,
            underground_height,
            sky_height,
        );
    ...
    pub fn render_loop(&mut self) {
        if self.unbuilt_models.len() != 0 {
            if self.unbuilt_models[0].3 && !self.unbuilt_models[0].4 {
                self.chunk_grid[self.unbuilt_models[0].0][self.unbuilt_models[0].1].regenerate(
                    &mut self.change_block,
                    self.unbuilt_models[0].0,
                    self.unbuilt_models[0].1,
                    &mut self.set_blocks,
                );
            ...
            for i in 0..self.unbuilt_models.len() {
                if self.unbuilt_models[i].3 && !self.unbuilt_models[i].4 {
                    self.chunk_grid[self.unbuilt_models[i].0][self.unbuilt_models[i].1].regenerate(
                        &mut self.change_block,
                        self.unbuilt_models[i].0,
                        self.unbuilt_models[i].1,
                        &mut self.set_blocks,
                    );
            ...
            self.build_mesh
                .push((self.unbuilt_models[0].0, self.unbuilt_models[0].1));
        ...
        if self.change_block.len() != 0 && self.unbuilt_models.len() == 0 {
            for i in 0..self.change_block.len() {
                self.chunk_grid[self.change_block[i].0][self.change_block[i].1].blocks
                    [self.change_block[i].2][self.change_block[i].3][self.change_block[i].4]
                    .id = self.change_block[i].5;
                check_blocks_around_block(
                    self,
                    self.change_block[i].0,
                    self.change_block[i].1,
                    self.change_block[i].2,
                    self.change_block[i].3,
                    self.change_block[i].4,
                );
            }
            ...
            for k in 0..self.unbuilt_models.len() {
                self.build_mesh
                    .push((self.unbuilt_models[k].0, self.unbuilt_models[k].1));
            }
    ...
    pub fn draw(
        ...
        let loaded_textures = self.loaded_textures.as_ref().unwrap();
        ...
        for i in 0..self.chunk_grid.len() {
            for k in 0..self.chunk_grid[i].len() {
                ...
                let uniforms = uniform! {
                    tex: glium::uniforms::Sampler(loaded_textures, behavior),
                    model: model,
                    view: view,
        ...
        for i in 0..self.chunk_grid.len() {
            for k in 0..self.chunk_grid[i].len() {
                if let Some(chunk_model) = self.chunk_grid[i][k].vertex_transparent.as_ref() {
                    let uniforms = uniform! {
                        tex: glium::uniforms::Sampler(loaded_textures, behavior),
                        model: model,
                        view: view,
                                ...
                                if i == 0 {
                                    ...
                                    for k in 0..length {
                                        let mut new_position =
                                            self.chunk_grid[length - 1][k].position.clone();
                                        let mut grid =
                                            self.chunk_grid[length - 1][k].get_grid().clone();
                        ...
                        4 => {
                            ...
                            if i != self.chunk_grid.len() - 1 {
                                if i == 0 {
                                    ...
                                    for k in 0..length {
                                        let mut new_position =
                                            self.chunk_grid[0][k].position.clone();
                        ...
                        match change_direction {
                            1 => {
                                ...
                                if k == 0 {
                                    ...
                                    let mut new_position =
                                        self.chunk_grid[i][length - 1].position.clone();
                                    let mut grid =
                                        self.chunk_grid[i][length - 1].get_grid().clone();
                                    ...
                                    self.unbuilt_models
                                        .push((i, length - 1, true, false, false));
                                } else {
                                    if k != length - 1 {
                            ...
                            3 => {
                                ...
                                if k == 0 {
                                    ...
                                    self.unbuilt_models
                                        .push((i, length - 2, true, false, false));
    ...
    pub fn destroy_block(&mut self, camera_front: &[f32; 3], camera_pos: &[f32; 3]) {
        let (position, mut end, direction) =
            (camera_pos.clone(), camera_pos.clone(), camera_front.clone());
        while distance_3d(&position.clone(), &end.clone()) < 6.0 {
            let block_index = get_block(&self, &end);
            if block_index.0 != 9999
                && block_index.1 != 9999
                && block_index.2 != 9999
                && block_index.3 != 9999
                && block_index.4 != 9999
            {
                ...
                let block = &mut self.chunk_grid[block_index.0][block_index.1].blocks
                    [block_index.2][block_index.3][block_index.4];
    ...
    pub fn place_block(
        ...
        while distance_3d(&position.clone(), &end.clone()) < 6.0 {
            ...
            if block_index.0 != 9999
                ...
                && block_index.4 != 9999
            {
                let block = &self.chunk_grid[block_index.0][block_index.1].blocks[block_index.2]
                    [block_index.3][block_index.4];

                if Block::is_air(&block) || Block::is_water(&block) {
                    last_air_block_index = block_index.clone();
                } else {
                    if is_player_in_block_location(
                        self,
                        camera_pos,
                        player_height,
                        last_air_block_index.0,
                        last_air_block_index.1,
                        last_air_block_index.2,
                        last_air_block_index.3,
                        last_air_block_index.4,
                    ) {
                        break;
                    }
                    ...
                    //Setting block in saved blocks
                    let grid =
                        self.chunk_grid[last_air_block_index.0][last_air_block_index.1].get_grid();
                    check_and_set_block(
                        &mut self.set_blocks,
                        grid.0,
                        grid.1,
                    ...
                    let block = &mut self.chunk_grid[last_air_block_index.0]
                        [last_air_block_index.1]
                        .blocks[last_air_block_index.2][last_air_block_index.3]
                        [last_air_block_index.4];
                    block.visible = false;
    ...
    pub fn move_to_direction(
        ...
        if block_index.0 != 9999
            ...
            && block_index.4 != 9999
        {
            if self.chunk_grid[block_index.0][block_index.1].blocks[block_index.2][block_index.3]
                [block_index.4]
                .is_water()
            {
                block_up = 1;
        ...
        block_index = get_block_or_water(
            self,
            &[
                desired_position[0],
                desired_position[1] - player_height,
                desired_position[2],
            ],
            margin_for_player,
        );
            ...
            if Block::is_air(&block) {
                ...
                if status == 2 {
                    ...
                    self.get_spawn_location(
                        &[camera_pos[0], camera_pos[1] - 1.0, camera_pos[2]],
                        1 as usize,
                    )
                }
            } else {
...
fn check_and_set_block(
    set_blocks: &mut HashMap<String, u8>,
    grid_x: i32,
    grid_z: i32,
    i: usize,
    k: usize,
...
fn is_player_in_block_location(
    world: &World,
    camera_pos: &[f32; 3],
    player_height: f32,
    i: usize,
    ...
    let block_index_down = get_block_or_air(
        world,
        &[camera_pos[0], camera_pos[1] - player_height, camera_pos[2]],
    );

    if block_index_up.0 == i
        && block_index_up.1 == k
        && block_index_up.2 == j
        && block_index_up.3 == l
        && block_index_up.4 == m
    ...
    if block_index_down.0 == i
        && block_index_down.1 == k
        && block_index_down.2 == j
        && block_index_down.3 == l
        && block_index_down.4 == m
    {
        return true;
    }
    return false;
...
fn distance_3d(arr1: &[f32; 3], arr2: &[f32; 3]) -> f32 {
    (f32::powi(arr1[0] - arr2[0], 2)
        + f32::powi(arr1[1] - arr2[1], 2)
        + f32::powi(arr1[2] - arr2[2], 2))
    .sqrt()
}
    ...
    for i in 0..world.chunk_grid.len() {
        for k in 0..world.chunk_grid[i].len() {
            ...
            let distance = distance_2d(
                ...
                &[end[0].clone(), end[2].clone()],
            );
            let distance_x = (f32::powi(position[0] - end[0], 2)).sqrt();
            let distance_y = (f32::powi(position[2] - end[2], 2)).sqrt();

            if distance < min
                && distance_x < world.chunk_width as f32 / 2.0
                && distance_y < world.chunk_width as f32 / 2.0
            {
                min = distance;
                index_i = i;
                index_k = k;
        ...
        for j in 0..world.chunk_grid[index_i][index_k].blocks.len() {
            for l in 0..world.chunk_grid[index_i][index_k].blocks[j].len() {
                for m in 0..world.chunk_grid[index_i][index_k].blocks[j][l].len() {
                    if world.chunk_grid[index_i][index_k].blocks[j][l][m].visible
                        && world.chunk_grid[index_i][index_k].blocks[j][l][m].id != 3
                    {
                        let position = world.chunk_grid[index_i][index_k].blocks[j][l][m]
                            .position
                            .clone();
    ...
    if min != 9999.0 {
        ...
        for j in 0..world.chunk_grid[index_i][index_k].blocks.len() {
            for l in 0..world.chunk_grid[index_i][index_k].blocks[j].len() {
                for m in 0..world.chunk_grid[index_i][index_k].blocks[j][l].len() {
                    if world.chunk_grid[index_i][index_k].blocks[j][l][m].visible
                        || world.chunk_grid[index_i][index_k].blocks[j][l][m].is_water()
                    {
                        let position = world.chunk_grid[index_i][index_k].blocks[j][l][m]
                            .position
                            .clone();
                        ...
                        if distance < min
                            && distance_x < 0.5 + margin_for_player
                            && distance_y < 0.5
                            && distance_z < 0.5 + margin_for_player
                        {
                            index_j = j;
                            index_l = l;
                            index_m = m;
...
fn generate_chunks(
    display: &glium::Display<WindowSurface>,
    chunk_grid: &mut Vec<Vec<Chunk>>,
    change_block: &mut Vec<(usize, usize, usize, usize, usize, u8)>,
    camera_position: &[f32; 3],
    square_chunk_width: &usize,
    render_out_from_player: &usize,
    world_gen_seed: &u32,
    mid_height: &u8,
    set_blocks: &mut HashMap<String, u8>,
    underground_height: &u8,
    ...
    let adjustment = (*render_out_from_player as f32 / 2.0).floor() as f32
        * square_chunk_width.clone() as f32
        + (*square_chunk_width as f32 / 2.0);
...
fn distance(max_distance: f32, point1: &[f32; 3], point2: &[f32; 3]) -> bool {
    return (f32::powi(point1[0].clone() - point2[0].clone(), 2))
        .sqrt()
        .floor()
        <= max_distance
        && (f32::powi(point1[2].clone() - point2[2].clone(), 2))
            .sqrt()
            .floor()
            <= max_distance;
}
...
fn get_direction(point1: &[f32; 3], point2: &[f32; 3]) -> usize {
    ...
    if !bigger && !x_negative {
        // Positive x
        1
    } else if !bigger && x_negative {
        // Negative x
        3
    } else if bigger && !z_negative {
        // Positive z
        2
    } else {
        // Negative z
...
fn setup_texture(world: &mut World, display: &glium::Display<WindowSurface>) {
    // let image = image::open(&std::path::Path::new("resources\\TextureTemplate.png")).unwrap().into_rgba8();
    let image = image::load(
        Cursor::new(&include_bytes!("../resources/TextureTemplate.png")),
    ...
    world.loaded_textures = Some(texture);
}
...
fn check_block_sides(
    ...
    if chunk_grid[i][k].blocks[j][l][m].is_air() {
        ...
        if m == 0 {
            y_block_flag = 0
        } else if m == chunk_grid[i][k].blocks[j][l].len() - 1 {
            y_block_flag = 2
        } else {
            y_block_flag = 1
        }; //Y axis
        ...
        if z_block_flag == 2 {
            if z_chunk_flag == 2 || chunk_grid.len() - 1 == 0 && z_chunk_flag == 0 {
                ...
                if chunk_grid[i + 1][k].blocks[0][l][m].is_air()
                    || block_id != 3 && chunk_grid[i + 1][k].blocks[0][l][m].is_water()
                {
                    sides.push(true);
                } else {
                    sides.push(false);
            ...
            if chunk_grid[i][k].blocks[j + 1][l][m].is_air()
                || block_id != 3 && chunk_grid[i][k].blocks[j + 1][l][m].is_water()
            {
                sides.push(true);
            } else {
                sides.push(false);
        ...
        if z_block_flag == 0 {
            if z_chunk_flag == 0 {
                ...
                if chunk_grid[i - 1][k].blocks[chunk_width - 1][l][m].is_air()
                    || block_id != 3
                        && chunk_grid[i - 1][k].blocks[chunk_width - 1][l][m].is_water()
                {
                    sides.push(true);
                } else {
                    sides.push(false);
        ...
        if x_block_flag == 2 {
            if x_chunk_flag == 2 || chunk_grid.len() - 1 == 0 && x_chunk_flag == 0 {
                ...
                if chunk_grid[i][k + 1].blocks[j][0][m].is_air()
                    || block_id != 3 && chunk_grid[i][k + 1].blocks[j][0][m].is_water()
                {
                    sides.push(true);
                } else {
                    sides.push(false);
            ...
            if chunk_grid[i][k].blocks[j][l + 1][m].is_air()
                || block_id != 3 && chunk_grid[i][k].blocks[j][l + 1][m].is_water()
            {
                sides.push(true);
            } else {
                sides.push(false);
        ...
        if x_block_flag == 0 {
            if x_chunk_flag == 0 {
                ...
                if chunk_grid[i][k - 1].blocks[j][chunk_width - 1][m].is_air()
                    || block_id != 3
                        && chunk_grid[i][k - 1].blocks[j][chunk_width - 1][m].is_water()
                {
                    sides.push(true);
                } else {
                    sides.push(false);
        ...
        if y_block_flag == 0 {
            ...
            if chunk_grid[i][k].blocks[j][l][m - 1].is_air()
                || block_id != 3 && chunk_grid[i][k].blocks[j][l][m - 1].is_water()
            {
                sides.push(true);
            } else {
                sides.push(false);
...
fn check_blocks_around_block(world: &mut World, i: usize, k: usize, j: usize, l: usize, m: usize) {
    ...
    //Check up
    if world.chunk_grid[i][k].blocks[j][l].len() != 1
        && m != world.chunk_grid[i][k].blocks[j][l].len() - 1
        && m < world.chunk_grid[i][k].blocks[j][l].len() + 1
    {
        check_block_sides(
            &mut world.chunk_grid,
    ...
    if j != 0 {
        ...
        if !world
            .unbuilt_models
            .contains(&(i - 1, k, false, false, false))
        {
    ...
    if l != 0 {
        ...
        if !world
            .unbuilt_models
            .contains(&(i, k - 1, false, false, false))
        {
...
fn build_mesh(world: &mut World, display: &glium::Display<WindowSurface>) {
    for i in 0..world.chunk_grid.len() {
        for k in 0..world.chunk_grid[i].len() {
...
fn push_unbuilt_to_start(
    unbuilt: (usize, usize, bool, bool, bool),
    vector: &mut Vec<(usize, usize, bool, bool, bool)>,
) {
